...
//            ->groupBy('zone_id')
    ...
    public function edit(JoeysZoneSchedule $schedule)
    {
        $joeys = Joey::where('on_duty', '=', 1)->whereNull('deleted_at')->get();
        $zones = Zones::all();
        $joeyZoneSchedule = $this->joeyZoneScheduleRepository->find($schedule->id);
        return view('front.schedule.edit', compact('joeyZoneSchedule', 'joeys', 'zones'));
    }

    public function update(Request $request, JoeysZoneSchedule $schedule)
    {

        $joeyZoneSchedule = JoeysZoneSchedule::with('schedule', 'joey')->findOrFail($schedule->id);
        $data = [
            'zone_id' => $request->get('zone_id'),
            'start_time' => ($request->get('start_time') == null) ? $joeyZoneSchedule->schedule->start_time : $request->get('start_time'),
            'end_time' => ($request->get('end_time') == null) ? $joeyZoneSchedule->schedule->end_time : $request->get('end_time'),
            'capacity' => $request->get('capacity'),
        ];

        $this->zoneScheduleRepository->update($schedule->schedule->id, $data);

        $joeySchedule = [
            'joey_id' => $request->get('joey_id'),
            'zone_schedule_id' => $schedule->zone_schedule_id,
        ];

        $this->joeyZoneScheduleRepository->update($schedule->id, $joeySchedule);

        return redirect()->route('schedule.index')->with('success', 'Schedule updated successfully.');
    }

    public function destroy(JoeysZoneSchedule $schedule)
    {
        $schedule->delete();
